...
document.addEventListener('DOMContentLoaded', () => {
    ...
    allDivs.forEach(div => {
        if (div.id === 'someId') {
            div.classList.add('isAwesome');
        } else {
            div.classList.remove('isAwesome');
        }
    });
    */
    ...
    // Subscribe to the search button click and call searchCards with searchBox's value when that occurs
    searchButton.addEventListener('click', event => {
        console.log('Search clicked with current text of ' + searchBox.value, event);

        searchCards(searchBox.value);
    });
    /*
...
function addCardForQuestion(quest) {
    ...
    article.addEventListener('dblclick', (event) => {
        console.log('Double click works?', event);

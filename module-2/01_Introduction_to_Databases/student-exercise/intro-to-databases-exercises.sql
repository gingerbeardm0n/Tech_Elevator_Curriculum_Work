...
-- 8. The name, continent, population, GNP, and average life expectancy 
--    of all countries with an average life expectancy greater than 80 years (5 rows)
...
-- 15. The name, population, and GNP of all countries with a GNP greater than $1 trillion dollars and 
--a population of less than 100 million people (4 rows)
...
-- 18. The name and average life expectancy of all countries on the continent of Oceania for 
-- which an average life expectancy has been provided (i.e. not equal to null) (20 rows)

SELECT name, lifeexpectancy FROM country
WHERE continent = 'Oceania'
AND lifeexpectancy IS NOT NULL;

-- 19. The name of all countries on the continent of Oceania for which an average life expectancy has 
-- not been provided (i.e. equal to null) (8 rows)

SELECT name FROM country
WHERE continent = 'Oceania'
AND lifeexpectancy IS NULL;

-- 20. The name, continent, GNP, and average life expectancy of all countries that have an 
-- average life expectancy of at least 70 years and a GNP between $1 million and $100 million dollars (3 rows)
...
SELECT gnp AS 'GNP in millions of Euros', population, 
(gnp*1000000*1.13)/population AS 'GNP per person in US Dollars' FROM country
WHERE continent = 'Europe'

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)

SELECT name, (2020 - indepyear) AS 'Years since independance' FROM country
